...
function RandevuPage() {
  ...
  // TODO: Buton tıkladığında  refetch / rerender olmalı

  useEffect(() => {
    async function fetchData() {
      try {
        const [todayRes, pastRes, futureRes] = await Promise.all([
          axios.get(`${baseURL}/api/randevu/doktor/${doktorId}/gunluk`),
          axios.get(`${baseURL}/api/randevu/doktor/${doktorId}/gecmis`),
          axios.get(`${baseURL}/api/randevu/doktor/${doktorId}/gelecek`),
        ]);
        setData({
          today: todayRes.data,
          past: pastRes.data,
          ...
          {paginate(items, currentPage[tabKey]).map((item, index) => (
            <tr key={index}>
              ...
              <td>
                ...
                {/* TODO: Buton İşlevleri eklenecek */}
                ...
                <Container className="d-flex">
                  {showButtons && item.durum === 'alindi' && (
                    <>
                      {tabKey === "today" && (
                        <>
                          <Button variant="success" className="m-1 btn-sm">Tamamlandı</Button>
                          <Button variant="danger" className="m-1 btn-sm">İptal Edildi</Button>
                        </>
                      )}
                      {tabKey === "future" && (
                        <Button variant="danger" className="m-1 btn-sm">İptal Edildi</Button>
  ...
  return (
    <Container className="d-flex justify-content-center">
      <div className="card shadow rounded-4 w-100" style={{ width: '90%', minWidth: 'min-content' }}>
